...
class AindRigDataMapper(ads.AindDataSchemaRigDataMapper):
    ...
    @classmethod
    def build_runner(cls) -> Callable[[Launcher[{{ _python_class_prefix }}Rig, Any, Any]], "AindRigDataMapper"]:
        def _new(
            launcher: Launcher[{{ _python_class_prefix }}Rig, Any, Any],
        ) -> "AindRigDataMapper":
    ...
    @classmethod
    def _map(cls, rig: {{ _python_class_prefix }}Rig) -> instrument.Instrument:
